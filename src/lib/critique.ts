 ...
 * 講評データの品質チェック
 */
export function validateCritiqueQuality(critiqueData: {
  technique: string;
  composition: string;
  color: string;
}): {
  ...
  // 各項目の内容チェック
  if (!critiqueData.technique || critiqueData.technique.trim().length === 0) {
    issues.push("技術面の講評が空です");
  }
  if (
    !critiqueData.composition ||
    critiqueData.composition.trim().length === 0
  ) {
    issues.push("構図面の講評が空です");
  }
  if (!critiqueData.color || critiqueData.color.trim().length === 0) {
    issues.push("色彩面の講評が空です");
  }

  // 文字数チェック（極端に短い・長い場合）
  if (critiqueData.technique && critiqueData.technique.length < 20) {
    issues.push("技術面の講評が短すぎます");
  }
  if (critiqueData.composition && critiqueData.composition.length < 20) {
    issues.push("構図面の講評が短すぎます");
  }
  if (critiqueData.color && critiqueData.color.length < 20) {
    issues.push("色彩面の講評が短すぎます");
  }

  if (critiqueData.technique && critiqueData.technique.length > 300) {
    issues.push("技術面の講評が長すぎます");
  }
  if (critiqueData.composition && critiqueData.composition.length > 300) {
    issues.push("構図面の講評が長すぎます");
  }
  if (critiqueData.color && critiqueData.color.length > 300) {
    issues.push("色彩面の講評が長すぎます");
  }

  // 同じ内容の重複チェック
  if (
    critiqueData.technique === critiqueData.composition ||
    critiqueData.technique === critiqueData.color ||
    critiqueData.composition === critiqueData.color
  ) {
    issues.push("講評内容に重複があります");
  }
