...
describe('processImage', () => {
  ...
  describe('境界値テスト', () => {
    it('最大ファイルサイズ（10MB）の画像を処理できる', async () => {
      const mockLargeFile = new File(['x'.repeat(10 * 1024 * 1024)], 'large.jpg', { 
        type: 'image/jpeg',
        lastModified: Date.now()
      });
  ...
  describe('異常系', () => {
    it('サポートされていないファイル形式でエラーを投げる', async () => {
      ...
      await expect(processImage(mockUnsupportedFile))
        .rejects
        .toThrow('サポートされていないファイル形式です: text/plain');
    });

    it('ファイルサイズが制限を超える場合エラーを投げる', async () => {
      const mockTooLargeFile = new File(['x'.repeat(11 * 1024 * 1024)], 'toolarge.jpg', { 
        type: 'image/jpeg',
        lastModified: Date.now()
      });

      await expect(processImage(mockTooLargeFile))
        .rejects
        .toThrow('ファイルサイズが制限を超えています（最大10MB）');
    });
    ...
    it('空のファイルでエラーを投げる', async () => {
      ...
      await expect(processImage(mockEmptyFile))
        .rejects
        .toThrow('無効なファイルです: ファイル名が空です');
    });
    ...
    it('圧縮処理エラーを適切に処理する', async () => {
      ...
      await expect(processImage(mockFile))
        .rejects
        .toThrow('画像処理に失敗しました: Compression failed');
    });
  });
});