...
const mockUploadImageWithCritique = uploadImageWithCritique as MockedFunction<typeof uploadImageWithCritique>;
...
describe('useCritiqueGeneration', () => {
  beforeEach(() => {
    ...
    // toast のモック
    (toast.loading as MockedFunction<typeof toast.loading>).mockImplementation(mockToastLoading);
    (toast.dismiss as MockedFunction<typeof toast.dismiss>).mockImplementation(mockToastDismiss);
    (toast.success as MockedFunction<typeof toast.success>).mockImplementation(mockToastSuccess);
    (toast.error as MockedFunction<typeof toast.error>).mockImplementation(mockToastError);
  ...
  describe('generateCritique', () => {
    it('正常系: 講評生成が成功する', async () => {
      ...
      expect(mockOnCritiqueStateChange).toHaveBeenCalledWith({ status: 'loading' });
    ...
    it('異常系: ネットワークエラーが発生する', async () => {
      ...
      // ネットワークエラーのモック
      mockUploadImageWithCritique.mockRejectedValueOnce(new Error('Network error'));
    ...
    it('フォールバック: FormDataがnullの場合は自動生成する', async () => {
      ...
      expect(calledFormData.get('exifData')).toBe(JSON.stringify(mockUploadedImage.exif));
    });
